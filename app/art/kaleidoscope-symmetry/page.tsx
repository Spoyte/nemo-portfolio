...
import { kaleidoscopeSymmetry } from "@/lib/art/kaleidoscope-symmetry";
...
export default function KaleidoscopeSymmetryPage() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (isPlaying) {
        time += 16;
        kaleidoscopeSymmetry.generate(ctx, params, time);
      }
  ...
  const randomize = useCallback(() => {
    setParams({
      ...
      layers: Math.floor(1 + Math.random() * 7),
      complexity: 10 + Math.random() * 90,
      colorShift: 0 + Math.random() * 360,
      mirrorMode: ["reflect", "rotate", "both"][Math.floor(Math.random() * 3)],
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-light tracking-tight mb-2">Kaleidoscope</h1>
          <p className="text-slate-400 max-w-lg mx-auto">
            Recursive mirrored patterns with rotational symmetry — like looking through a kaleidoscope (seeded)
          </p>
        </div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-800">
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                                <button
                  onClick={() => setParams(prev => ({ ...prev, mirrorMode: "reflect" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.mirrorMode === "reflect"
                ...
                <button
                  onClick={() => setParams(prev => ({ ...prev, mirrorMode: "rotate" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.mirrorMode === "rotate"
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"
                      : "bg-slate-800/50 text-slate-400 hover:bg-slate-800 border border-transparent"
                    }
                  `}
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 text-xs text-slate-500">
              <p>Recursive mirrored patterns with rotational symmetry — like looking through a kaleidoscope (seeded)</p>
            </div>
          </div>
        </div>
      </div>
