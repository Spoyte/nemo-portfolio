...
export default function DlaPage() {
  ...
  const randomize = useCallback(() => {
    setParams({
      params: 500 + Math.random() * 4500,
      stickiness: 0.5 + Math.random() * 2.5,
      branchFactor: 0.3 + Math.random() * 1.2,

      animationSpeed: 0 + Math.random() * 5,
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-light tracking-tight mb-2">Diffusion Limited Aggregation</h1>
          <p className="text-slate-400 max-w-lg mx-auto">
            Organic fractal growth through particle random walks (seeded)
          </p>
        </div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-800">
            ...
            <div className="mb-5">
              ...
              <Slider
                ...
                onValueChange={([v]) => setParams(prev => ({ ...prev, stickiness: v }))}
                min={0.5}
                max={3}
                step={0.1}
                className="w-full"
            ...
            <div className="mb-5">
              <label className="flex items-center gap-2 text-sm font-medium text-slate-300 mb-2">
                <SlidersHorizontal className="w-4 h-4" />
                BranchFactor: {params.branchFactor.toFixed(2)}
              </label>
              <Slider
                value={[params.branchFactor]}
                onValueChange={([v]) => setParams(prev => ({ ...prev, branchFactor: v }))}
                min={0.3}
                max={1.5}
                step={0.1}
                className="w-full"
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 text-xs text-slate-500">
              <p>Organic fractal growth through particle random walks (seeded)</p>
            </div>
          </div>
        </div>
      </div>
