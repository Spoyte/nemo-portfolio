...
import { orbitalMechanics } from "@/lib/art/orbital-mechanics-generator";
...
export default function OrbitalMechanicsGeneratorPage() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (isPlaying) {
        time += 16;
        orbitalMechanics.generate(ctx, params, time);
      }
  ...
  const randomize = useCallback(() => {
    setParams({
      ...
      gravityStrength: 10 + Math.random() * 140,
      trailLength: 10 + Math.random() * 90,
      colorScheme: ["cosmic", "solar", "nebula", "monochrome"][Math.floor(Math.random() * 4)],
    });
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-light tracking-tight mb-2">Orbital Mechanics</h1>
          <p className="text-slate-400 max-w-lg mx-auto">
            Gravitational particle simulation with decaying trails
          </p>
        </div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-800">
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() => setParams(prev => ({ ...prev, colorScheme: "solar" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.colorScheme === "solar"
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"
                      : "bg-slate-800/50 text-slate-400 hover:bg-slate-800 border border-transparent"
                    }
                  `}
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 text-xs text-slate-500">
              <p>Gravitational particle simulation with decaying trails</p>
            </div>
          </div>
        </div>
      </div>
