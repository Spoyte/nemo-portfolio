...
import { metaballsDefaultParams } from "@/lib/art/metaballs";
...
export default function MetaballsPage() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (isPlaying) {
        time += 16;
        metaballsDefaultParams.generate(ctx, params, time);
      }
  ...
  const randomize = useCallback(() => {
    setParams({
      color: Math.floor(2 + Math.random() * 6),
      ballRadius: 30 + Math.random() * 70,
      threshold: 0.5 + Math.random() * 1.5,
      speed: 0.2 + Math.random() * 1.8,
      colorScheme: ["neon", "ocean", "sunset", "forest", "magma", "ice", "gold", "midnight"][Math.floor(Math.random() * 8)],
      wireframe: ["off", "on"][Math.floor(Math.random() * 2)],
      glow: ["off", "on"][Math.floor(Math.random() * 2)],
      trails: 0 + Math.random() * 0.95,
      resolution: 2 + Math.random() * 6,
    });
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          <h1 className="text-4xl font-light tracking-tight mb-2">Metaballs</h1>
          <p className="text-slate-400 max-w-lg mx-auto">
            Organic implicit surfaces created by blending field functions. Blobby shapes that merge and separate fluidly.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr,340px] gap-8 items-start">
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-800">
            ...
            <div className="mb-5">
              ...
              <Slider
                value={[params.ballRadius]}
                onValueChange={([v]) => setParams(prev => ({ ...prev, ballRadius: v }))}
                min={30}
                max={100}
                step={5}
                className="w-full"
            ...
            <div className="mb-5">
              ...
              <Slider
                ...
                onValueChange={([v]) => setParams(prev => ({ ...prev, threshold: v }))}
                min={0.5}
                max={2.0}
                step={0.1}
                className="w-full"
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                                <button
                  onClick={() => setParams(prev => ({ ...prev, wireframe: "off" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.wireframe === "off"
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"
                      : "bg-slate-800/50 text-slate-400 hover:bg-slate-800 border border-transparent"
                    }
                  `}
                ...
                <button
                  onClick={() => setParams(prev => ({ ...prev, wireframe: "on" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.wireframe === "on"
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"
                      : "bg-slate-800/50 text-slate-400 hover:bg-slate-800 border border-transparent"
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                                <button
                  onClick={() => setParams(prev => ({ ...prev, glow: "off" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.glow === "off"
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"
                      : "bg-slate-800/50 text-slate-400 hover:bg-slate-800 border border-transparent"
                ...
                <button
                  onClick={() => setParams(prev => ({ ...prev, glow: "on" }))}
                  className={`px-3 py-2 rounded-lg text-sm transition-all capitalize
                    ${params.glow === "on"
                      ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50"
                      : "bg-slate-800/50 text-slate-400 hover:bg-slate-800 border border-transparent"
            ...
            <div className="mb-5">
              <label className="flex items-center gap-2 text-sm font-medium text-slate-300 mb-2">
                <SlidersHorizontal className="w-4 h-4" />
                Trails: {params.trails.toFixed(2)}
              </label>
              <Slider
                value={[params.trails]}
                onValueChange={([v]) => setParams(prev => ({ ...prev, trails: v }))}
                min={0}
                max={0.95}
                step={0.05}
                className="w-full"
            ...
            <div className="mb-5">
              ...
              <Slider
                ...
                onValueChange={([v]) => setParams(prev => ({ ...prev, resolution: v }))}
                min={2}
                max={8}
                step={2}
                className="w-full"
            ...
            <div className="mt-6 pt-6 border-t border-slate-800 text-xs text-slate-500">
              <p>Organic implicit surfaces created by blending field functions. Blobby shapes that merge and separate fluidly.</p>
            </div>
          </div>
        </div>
      </div>
